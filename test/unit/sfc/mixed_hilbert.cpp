...
void inversionTest2D3D()
{
    ...
    const int level_1D = 2;

    for (int i = 0; i < numKeys; ++i)
    {
        KeyType hilbertKey = iHilbert2DMixed<KeyType>(x[i], y[i], z[i], level_1D, 0);
        auto [a, b, c]     = decodeHilbert2DMixed(hilbertKey, level_1D, 0);
